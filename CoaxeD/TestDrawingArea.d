...
class TestDrawingArea : VBox
{
	...
	class TestDrawing : DrawingArea
	{
		...
		public bool onButtonPress(Event event, Widget widget)
		{
			debug(trace)
			{
				version(Tango) Stdout("button DOWN").newline;
			...
			if ( event.type == EventType.BUTTON_PRESS && event.button.button == 1 )
			{
				debug(trace) 
				{
					version(Tango) Stdout("Button 1 down").newline;
		...
		public bool onButtonRelease(Event event, Widget widget)
		{
			debug(trace) 
			{
				version(Tango) Stdout("button UP").newline;
			...
			if ( event.type == EventType.BUTTON_RELEASE && event.button.button == 1 )
			{
				debug(trace)
				{
					version(Tango) Stdout("button 1 UP").newline;
		...
		public void backSpinChanged(SpinButton spinButton)
		{
			...
			debug(trace) 
			{
				version(Tango) Stdout.format("backSpinChanged - entry {}", ++backSpinCount).newline;
			...
			debug(trace) 
			{
				version(Tango) Stdout("backSpinChanged - exit").newline;
		...
		public void drawPrimitive(int x, int y)
		{
			...
			debug(trace) 
			{
				version(Tango) Stdout.format("primitiveType = {}", primitiveType).newline;
		...
		private void drawPoints(Context context)
		{
			...
			debug(trace) 
			{
				version(Tango) Stdout.format("w,h = {} {}",width ,height).newline;
		...
		void onOperatorsChanged(ComboBoxText comboBoxText)
		{
			debug(trace) 
			{
				version(Tango) Stdout.format("CairoOperator = {}", comboBoxText.getActiveText()).newline;
