...
//		progressWindow.show();
		...
		for ( int i=0 ; i<imageFiles.length ;i++)
		{
			...
			debug(trace) 
			{
				version(Tango) Stdout.format("adding image {} to table at {},{}", fileName, col, row).newline;
	...
	void loadImages(Button button)
	{
		...
		if ( response == ResponseType.ACCEPT )
		{
			...
			for ( int i = 0; i<list.length() ; i++)
			{
				debug(trace)
				{
					version(Tango) Stdout.format("Testmage.loadImages.File selected = {}", Str.toString(cast(char*)list.nthData(i))).newline;
					else writefln("Testmage.loadImages.File selected = %s",	Str.toString(cast(char*)list.nthData(i)));
				}
